...
public class EmojiconsPopup extends PopupWindow implements ViewPager.OnPageChangeListener, EmojiconRecents {
    ...
    public EmojiconsPopup(View rootView, Context mContext) {
        ...
        setHeight((int) mContext.getResources().getDimension(R.dimen.keyboard_height));
        setBackgroundDrawable(null);

        unselectedTabIconColor = mContext.getResources().getColor(R.color.emojicons_unselected_tab);
        selectedTabIconColor = mContext.getResources().getColor(R.color.emojicons_selected_tab);
    }
    ...
    private View createCustomView() {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.emojicons, null, false);
        ...
        ImageButton backspaceButton = view.findViewById(R.id.emojis_backspace);
        backspaceButton.setOnTouchListener(new RepeatListener(1000, 50, new OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        ImageButton keyboardButton = view.findViewById(R.id.emojis_keyboard_image);
        ...
        switch (i) {
            ...
            case 5:
                if (mEmojiTabLastSelectedIndex >= 0 && mEmojiTabLastSelectedIndex < mEmojiTabs.length) {
                    tintUnselected(mEmojiTabLastSelectedIndex);
                }
                tintSelected(i);
    ...
    private void tintSelected(int tabIndex) {
        final ImageButton b = (ImageButton) mEmojiTabs[tabIndex];
        b.post(new Runnable() {
            @Override
            public void run() {
                b.setColorFilter(selectedTabIconColor);
            }
        });
    }

    private void tintUnselected(int tabIndex) {
        ((ImageButton) mEmojiTabs[tabIndex]).setColorFilter(unselectedTabIconColor);
    }
